...
def get_data_set(number, X_data, Y_data):
    ...
    DL = wfdb.rdrecord(file_path)
    print(DL.sig_name)
    record = wfdb.rdrecord(file_path, channel_names=['III'])
    # INCART数据集的channel_names为: ['I', 'II', 'III', 'AVR', 'AVL', 'AVF', 'V1', 'V2', 'V3', 'V4', 'V5', 'V6']
    if record is None:
        print(f"警告: {number}.dat的数据读取失败, 跳过...")
    ...
    while i < j:
        try:
            ...
            # 加上长度校验。当 R 波位置很靠近信号开头或结尾时，x_train会截取到不足300点的片段，导致后续 reshape(-1, 300) 报错。需要加防护
            if len(x_train) != 300:
                i += 1
                continue
...
def load_data(ratio, random_seed):
    ...
    X_train, X_test, y_train, y_test = train_test_split(data_set, label_set, test_size=ratio, random_state=random_seed, stratify=label_set) # 加了分层处理


    # ECG 信号通道间幅值差异较大，代码在小波去噪后没有归一化，因此在代码中加标准化处理（Z-score 或 min-max）：
    from sklearn.preprocessing import StandardScaler
    scaler = StandardScaler()
    X_train = scaler.fit_transform(X_train)  # 用训练集学习均值和标准差
    X_test = scaler.transform(X_test)        # 测试集只用训练集的参数来变换
...
def main():
    ...
    if os.path.exists(model_path) and not train:
        ...
        if pre_train:
            # model = tf.keras.models.load_model(filepath=model_path)
            model = tf.keras.models.load_model(filepath=model_path, custom_objects={'SparseFocalLoss': SparseFocalLoss,
                                                                                    'SparseFocalLoss_previous': SparseFocalLoss_previous})


        # 配置模型训练时的损失函数不用关注之前的预训练权重所用的损失函数
        # opt = tf.python.keras.optimizers.adam_v2.Adam(learning_rate= 0.00001)
        opt = tf.keras.optimizers.Adam(learning_rate=0.00001)  # tensorflow-2.4.0版本是这样写的优化器，现在用的就是2.4.0的tf（可以cuda加速），cudnn为8.0，cuda为11.0
        # opt = tf.keras.optimizers.legacy.Adam(learning_rate= 0.00001)  # tensorflow-2.10.0版本是这样写的优化器，但2.4.0就写法不一样
